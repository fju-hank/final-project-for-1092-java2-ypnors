...
public class Player21a implements Player {
    ...
    public Player21a(int cpt){
        int i;
        card_pt=cpt;//可以設定選用不同牌名
        card_cnt=0;//預設選用中文全名
        for (i=0;i<13;i=i+1)  this.card[i]=-1;
        card_value=0;//清理牌計數
        status=0;//設定收牌
    }
    ...
    public boolean isgo() //決定要不要再要牌
    {
        ...
        if (card_value<17) return true; //大於17點就不要牌了
        return false;
    }
